...
namespace codeTest
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			String oldBibleShortcutString = @"창 출 레 민 신 수 삿 룻 삼상 삼하 왕상 왕하 대상 대하 스 느 에 욥 시 잠 전 아 사 렘 애 겔 단 호 욜 암 옵 욘 미 나 합 습 학 슥 말";
			String newBibleShortcutString = @"마 막 눅 요 행 롬 고전 고후 갈 엡 빌 골 살전 살후 딤전 딤후 딛 몬 히 약 벧전 벧후 요일 요이 요삼 유 계";
			ArrayList bibleList = new ArrayList();
			String[] oldBibleList = oldBibleString.Split(' ');
			String[] newBibleList = newBibleString.Split(' ');
			String[] oldBibleShortcutList = oldBibleShortcutString.Split(' ');
			String[] newBibleShortcutList = newBibleShortcutString.Split(' ');
			...
			foreach (String bibleShortcut in oldBibleShortcutList)
				bibleList.Add(bibleShortcut);
			foreach (String bibleShortcut in newBibleShortcutList)
				bibleList.Add(bibleShortcut);
			...
			if(oldBibleShortcutList.Contains(bibleList[idx + 1]))
				Console.WriteLine("convert : " + oldBibleList[Array.IndexOf(oldBibleShortcutList, bibleList[idx + 1])]);
			else if(newBibleShortcutList.Contains(bibleList[idx + 1]))
				Console.WriteLine("convert : " + newBibleList[Array.IndexOf(newBibleShortcutList, bibleList[idx + 1])]);
			else
